// Netlify Function equivalent of api/export-pptx.js — same job, different
// handler signature and binary response format (base64 + isBase64Encoded).
...
function buildPlanDeck(pptxgen, data) {
  ...
  (data.channels || []).forEach((c) => {
    rows.push([{ text: c.name || "" }, { text: c.role || "" }, { text: c.kpi || "" }, { text: Number(c.budget_sar || 0).toLocaleString("en-US"), options: { align: "right" } }]);
  });
  ...
  (data.channels || []).forEach((c) => {
    creativeRows.push([{ text: c.name || "" }, { text: c.best_format || c.formats || "" }, { text: c.creative_recommendation || "" }]);
  });
...
function buildReportDeck(pptxgen, data, meta) {
  ...
  s.addChart(pptxgen.ChartType.doughnut,
    ...
    { x: 9.5, y: 4.0, w: 3.0, h: 2.4, chartColors: ["39C982", "8890B5", "F27272"], showLegend: true, legendPos: "b", legendFontSize: 9 }
  );
  ...
  (data.themes || []).forEach((t) => {
    rows.push([{ text: t.name || "" }, { text: t.sentiment || "" }, { text: `~${t.volume_pct || 0}%`, options: { align: "right" } }, { text: t.description || "" }]);
  });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    type = body.type; data = body.data; meta = body.meta || {};
  } catch (e) {
  ...
  try {
    ...
    const pres = type === "plan" ? buildPlanDeck(pptxgen, data) : buildReportDeck(pptxgen, data, meta);
    ...
    return {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="khottah-${type}.pptx"`,
      },
      body: buffer.toString("base64"),
      isBase64Encoded: true,
    };
  } catch (err) {
    return { statusCode: 500, body: JSON.stringify({ error: "PPTX generation failed", detail: String(err) }) };
  }
};
